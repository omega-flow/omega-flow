...
- [#43](https://github.com/omega-flow/omega-flow/pull/43) [`35f1ac6`](https://github.com/omega-flow/omega-flow/commit/35f1ac68fa7745a38dc0eebf681774103ebb1851) Thanks [@fones](https://github.com/fones)! - **BREAKING**: Remove `EventBusWorkflowScheduler` (and its config type). `SqsFifoWorkflowScheduler` is the scheduler for AWS deployments — it targets the SQS FIFO queue directly, preserving per-subject `MessageGroupId` ordering, which the EventBridge-bus route could not guarantee. Migrate by replacing `EventBusWorkflowScheduler({ client, eventBusArn, roleArn })` with `SqsFifoWorkflowScheduler({ client, queueArn, roleArn, messageGroupIdExtractor })` and pointing the schedule target role's IAM at `sqs:SendMessage` on the queue instead of `events:PutEvents` on the bus.

- [#37](https://github.com/omega-flow/omega-flow/pull/37) [`27c74ec`](https://github.com/omega-flow/omega-flow/commit/27c74ecf2990604a1421aa6274879c2380c52568) Thanks [@fones](https://github.com/fones)! - Add `SqsFifoWorkflowScheduler`: an EventBridge Scheduler-backed `WorkflowScheduler` that targets an SQS FIFO queue directly, setting `MessageGroupId` so Wait-node wake-ups stay strictly ordered and serialized with the webhook events for the same subject.
